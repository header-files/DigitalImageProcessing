...
img = cv.imread('../../img/DIP3E_Original_Images_CH04/Fig0445(a)(characters_test_pattern).tif', 0)
dft = cv.dft(np.float32(img), flags=cv.DFT_COMPLEX_OUTPUT)
dft_shift = np.fft.fftshift(dft)
magnitude_spectrum = 20 * np.log(cv.magnitude(dft_shift[:, :, 0], dft_shift[:, :, 1]))

mask = BLPF(img.shape, (img.shape[0] / 2, img.shape[1] / 2), 160, 2)
...
ax[2].set_title('布特沃斯低通滤波后频谱图')
ax[2].imshow(fshift_magnitude_spectrum, cmap='gray')
ax[2].axis('off')

ax[3].set_title('布特沃斯低通滤波处理后')
